...
void crb_array_add(CRB_Interpreter *inter, CRB_Object *obj, CRB_Value v) {
	...
	if (obj->u.array.size + 1 > obj->u.array.alloc_size) {
		...
		inter->heap.current_heap_size += (new_size - obj->u.array.alloc_size)
				* sizeof(CRB_Value);
...
void crb_array_resize(CRB_Interpreter *inter, CRB_Object *obj, int new_size) {
	...
	if (need_realloc) {
		...
		inter->heap.current_heap_size += (new_alloc_size
				- obj->u.array.alloc_size) * sizeof(CRB_Value);
...
static void gc_dispose_object(CRB_Interpreter *inter, CRB_Object *obj) {
	switch (obj->type) {
	case ARRAY_OBJECT:
		inter->heap.current_heap_size -= sizeof(CRB_Value)
				* obj->u.array.alloc_size;
